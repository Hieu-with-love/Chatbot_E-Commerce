...
//        }
    ...
    private void handleIntentBasedResponse(String userIntent, String originalPrompt) {
        ...
        switch (userIntent) {
            case "product_overview":
                handleProductOverview(originalPrompt);
                break;
            case "search_product":
    ...
    private void saveMessage(String content, String sender){
        addMessageToChat(content, Message.SENT_BY_BOT);
    ...
    private void extractSearchJsonThenSendToBackend(String originalPrompt) {
        ...
        String promptTemplateS = ChatGeminiUtils.readFileFromAssets(this, R.raw.search_product_prompt);
  ...
  // Cập nhật method generateFinalResponse để handle các intent khác nhau
    ...
    // Get 5 most recent pairs of user and bot messages from recycler view
    public void onBotResponseReceived(int sessionId) {
        ...
        for (int i = messageList.size()-1; i >= 0; i--){
            ...
            }else if (message.getSentBy()==Message.SENT_BY_BOT && lastUserMessage != null){
                recentPairs.add(new Pair<>(lastUserMessage, message.getContent()));
                lastUserMessage = null;
            }
            if (recentPairs.size() >= 5) break;
        }
    ...
    private void requestSummaryFromGemini(String conversationPrompt, SummaryCallback callback) {
        String summaryPre = ChatGeminiUtils.readFileFromAssets(this, R.raw.summary_prompt);
        String structuredPrompt = String.format(summaryPre, conversationPrompt);
    ...
    private void startNewChatSession() {
        String token = TokenManager.getToken(this);
        Log.d("Start new chat session", "Checking session" + token);
        chatbotApiService.checkExistsChatSession(token).enqueue(new retrofit2.Callback<ResponseData<ChatSessionResponse>>() {            @Override
            public void onResponse(retrofit2.Call<ResponseData<ChatSessionResponse>> call, retrofit2.Response<ResponseData<ChatSessionResponse>> response) {
                if (response.isSuccessful() && response.body() != null && response.body().getData() != null) {
                    ...
                    Log.e("ChatSession", "Failed to check session exists: response body is null or unsuccessful");
                    Toast.makeText(ChatGeminiActivity.this, "Failed to check session status", Toast.LENGTH_SHORT).show();
